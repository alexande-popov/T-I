...
- [1. Подходы к декомпозиции](#1-подходы-к-декомпозиции)
- [2. Акторы и источники](#2-акторы-и-источники)
  - [2.1. Различие](#21-различие)
  - [2.2. Акторы](#22-акторы)
  - [2.3. Источники (не акторы)](#23-источники-не-акторы)
  - [2.4. Роли Пользователя и Оператора](#24-роли-пользователя-и-оператора)
- [3. Workflows (рабочие процессы)](#3-workflows-рабочие-процессы)
  - [3.1. Интерфейсные процессы](#31-интерфейсные-процессы)
  - [3.2. Сквозные механизмы (инфраструктура)](#32-сквозные-механизмы-инфраструктура)
  - [3.3. Доменные процессы](#33-доменные-процессы)
  - [3.4. Упорядоченный список (от источника до пользователя)](#34-упорядоченный-список-от-источника-до-пользователя)
  - [3.5. Важные уточнения](#35-важные-уточнения)
- [4. Базовый компонент](#4-базовый-компонент)
  - [4.1. Критерии базовости](#41-критерии-базовости)
  - [4.2. Применение к компонентам](#42-применение-к-компонентам)
  - [4.3. Вывод](#43-вывод)
- [5. Логические компоненты](#5-логические-компоненты)
  - [5.1. Таблица компонентов](#51-таблица-компонентов)
  - [5.2. Граф зависимостей](#52-граф-зависимостей)
  - [5.3. Контракты компонентов (черновик)](#53-контракты-компонентов-черновик)
  - [5.4. Соответствие компонентов структуре кода](#54-соответствие-компонентов-структуре-кода)
  - [5.5. MVP: базовые компоненты](#55-mvp-базовые-компоненты)
  - [5.6. Корзины компонентов MVP](#56-корзины-компонентов-mvp)
  - [5.7. Связность логических компонентов и её снижение](#57-связность-логических-компонентов-и-её-снижение)
- [6. Сценарии по акторам](#6-сценарии-по-акторам)
  - [6.1. Действия Пользователя](#61-действия-пользователя)
  - [6.2. Действия Оператора](#62-действия-оператора)
  - [6.3. Действия Планировщика](#63-действия-планировщика)
  - [6.4. Действия Робота](#64-действия-робота)
  - [6.5. Внутренние шаги (обслуживают всех)](#65-внутренние-шаги-обслуживают-всех)
- [7. Матрица: сценарий × актор × компонент](#7-матрица-сценарий-×-актор-×-компонент)
- [8. User stories по акторам](#8-user-stories-по-акторам)
  - [8.1. Пользователь](#81-пользователь)
  - [8.2. Оператор](#82-оператор)
  - [8.3. Робот-исполнитель](#83-робот-исполнитель)
  - [8.4. Планировщик](#84-планировщик)
  - [8.5. Покрытие user stories компонентами](#85-покрытие-user-stories-компонентами)
- [9. Источники: контракты интеграции](#9-источники-контракты-интеграции)
  - [9.1. Брокер (T-Bank Invest API)](#91-брокер-t-bank-invest-api)
  - [9.2. Внешние данные](#92-внешние-данные)
- [10. Ключевые архитектурные правила](#10-ключевые-архитектурные-правила)
- [11. Что осталось уточнить](#11-что-осталось-уточнить)
...
Как связать эти компоненты слабее — в 5.7.
...
### 5.7. Связность логических компонентов и её снижение

5.2 показывает, **что** с чем связано. Этот раздел — **как связать слабее**, не меняя поведения. Снижение связности почти всегда окупается: компонент, который знает о двух других, ломаешь правкой в третьем.

#### 5.7.1. Принципы

| № | Принцип | Формулировка |
|---|---|---|
| П1 | Одна причина меняться | Если компонент правят по двум разным поводам, его надо разделить |
| П2 | Зависимость от абстракции | Компонент знает интерфейс, а не конкретный класс |
| П3 | События вместо вызовов | Источник не знает, кто его слушает |
| П4 | Фасад вместо прямого доступа | Интерфейс знает одну точку входа, а не пять компонентов |
| П5 | Инверсия зависимостей | Домен не зависит от инфраструктуры; инфраструктура подстраивается под домен |

П5 — это ровно правило 1 из 10 («зависимости только снизу вверх») в применении к хранению.

#### 5.7.2. Проблемы текущей связности

Источник — граф 5.2 и матрица 7.

| Проблема | Кто страдает | Почему плохо |
|---|---|---|
| Portfolio Store — узкое горлышко | все доменные компоненты | правка в API Store ломает всех сразу |
| Tax Engine читает Store напрямую | Tax Engine | зависит от схемы операций, а не от модели портфеля |
| Scheduler знает несколько компонентов | Scheduler | риск god object: расписание смешивается с логикой задач |
| Presenter знает несколько компонентов | Presenter | риск god object: форматирование смешивается с расчётом |
| CLI / Bot / Web знают ядро по компонентам | интерфейсы | каждый интерфейс дублирует знание о внутреннем устройстве |
| Store привязан к SQLite | Store | замена БД — переписывание обращений |

#### 5.7.3. Решение: репозиторий на тип данных

**Было:** один Portfolio Store со всеми методами.

**Стало:** несколько репозиториев, каждый про свой тип.

| Репозиторий | Контракт |
|---|---|
| `OperationRepository` | `save`, `get_by_ticker`, `get_by_period` |
| `SnapshotRepository` | `save`, `get_by_date`, `get_by_period` |
| `PriceRepository` | `save`, `get_by_ticker_and_date` |
| `AccountRepository` | `save`, `get_all` |

**Даёт:** Tax Engine зависит только от `OperationRepository`, Analytics — только от `SnapshotRepository`. Каждый интерфейс мал и меняется независимо.

**Теряет:** компонентов больше, каждый проще.

Владелец таблицы — тот репозиторий, который в неё пишет. Остальные читают через него: это развивает правило 5.4 про `models/` без обращений в БД и наблюдение 5.5 про то, что `db/` и `t_api/` встречаются только внутри `services/`.

#### 5.7.4. Решение: абстракция базы данных

**Было:** Store знает про SQLite.

**Стало:** Store работает с абстракцией, реализацию подставляет кто-то снаружи.
...
class PostgresDatabase(Database): ...  # на будущее
```

**Даёт:** замена БД — новый класс, а не переписывание репозиториев. Тесты берут in-memory реализацию вместо тестовой SQLite из PROJECT_PLAN 9.3.

#### 5.7.5. Решение: доменные структуры как контракт

**Было:** репозитории возвращают `dict` или строки БД.

**Стало:** репозитории возвращают dataclass'ы из `src/models/`.
...
**Даёт:** домен не знает схему БД, правка `operations` (4.2 из PROJECT_PLAN) не ломает Tax Engine. Контракты 5.3 перестают быть строками и становятся типами. Это же закрывает R35 из корзины Market Data Adapter в 5.6: протокол не протекает в домен.

#### 5.7.6. Решение: событийная шина

**Было:** Trade Executor вызывает Notification Service и Trade Journal напрямую.

**Стало:** Executor публикует событие, слушатели подписываются.
...
События: `OrderPlaced`, `OrderRejected`, `SnapshotCreated`, `AlertTriggered`.

**Даёт:** Executor не знает про Notification и Journal. Новый слушатель появляется без правки источника. Уведомления тестируются отдельно — это условие US-4, US-6 и US-15.

Правила 5 и 6 из 10 («проверка лимитов и ордера — разные компоненты», «уведомление и подтверждение — разные») при этом не ослабевают: разделение становится жёстче, потому что подписчик не может позвонить обратно в Executor.

#### 5.7.7. Решение: фасад над ядром

**Было:** CLI, Bot и Web знают Analytics, Portfolio Model, Store, Presenter.

**Стало:** интерфейсы знают только `InvestApp`.

```python
class InvestApp:
    def get_value_series(self, start, end) -> list[Point]: ...
    def get_structure(self, date, by) -> dict: ...
    def get_fifo(self, ticker) -> list[Lot]: ...
    def run_strategy(self, name) -> Plan: ...
    def confirm(self, trade_id) -> None: ...
    def kill_switch(self) -> None: ...
```

**Даёт:** интерфейсы не знают, как устроено ядро; внутренности меняются без правок CLI/Bot/Web; мокается один объект.

Это та же мысль, что в правиле 2 из 10 («ядро отдельно от интерфейсов»), но доведённая до одного импорта вместо четырёх. Методы фасада — это точки входа историй из 8: `get_value_series` = US-1, `get_structure` = US-2, `get_fifo` = US-3, `kill_switch` = US-7.

#### 5.7.8. Решение: Strategy Engine как чистый расчёт

**Было:** Strategy Engine формирует план и сам решает, что с ним делать.

**Стало:** чистая функция: состояние портфеля + правила → план. Без побочных эффектов.

**Даёт:** тесты вида «вход → выход», бэктесты из PROJECT_PLAN 2.2 прогоняются без торговли. Strategy не знает про Risk, Executor и Store.

#### 5.7.9. Решение: Risk Manager как композиция правил

**Было:** один компонент с набором проверок.

**Стало:** композиция правил.

```python
class Rule(Protocol):
    def check(self, order, context) -> Decision: ...


class PositionSizeRule(Rule): ...
class DailyLossRule(Rule): ...
class AvailableCashRule(Rule): ...
```

**Даёт:** новое правило — без правки остальных; каждое тестируется отдельно; AC «причина отказа логируется» из US-14 становится тривиальной — её даёт само правило.

#### 5.7.10. Целевая архитектура

Стрелка `A → B` читается как «A зависит от B».
...
flowchart TD
    ...
    APP["InvestApp · фасад"] --> PM["Portfolio Model"]
    APP --> AN["Analytics"]
    APP --> TE["Tax Engine"]
    APP --> SE["Strategy Engine"]

    PM --> OR["OperationRepository"]
    PM --> PRC["PriceRepository"]
    AN --> SNR["SnapshotRepository"]
    TE --> OR
    SE --> RM["Risk Manager"] --> TX["Trade Executor"]
    TX --> EB["EventBus"]
    EB --> NS["Notification Service"]
    EB --> TJ["Trade Journal"]

    OR --> DBA["Database · абстракция"]
    SNR --> DBA
    PRC --> DBA
    AR["AccountRepository"] --> DBA

    DBA --> LITE["SQLite"]
    DBA --> PG["Postgres"]
    DBA --> MEM["InMemory · тесты"]

    MDA["Market Data Adapter"] --> OR
    MDA --> PRC
    MDA --> AR

    classDef build fill:#fff2cc,stroke:#bf9000,color:#000
    class APP,EB,DBA,OR,SNR,PRC,AR build
```

Жёлтое — новое: фасад, шина, абстракция хранилища и репозитории. Остальное берётся из 5.1 без изменений, типы компонентов сохраняются.

#### 5.7.11. Что меняется по связности

| Аспект | Было | Стало |
|---|---|---|
| Доступ к данным | все → Store | каждый → свой репозиторий |
| Tax Engine | читает Store | зависит от `OperationRepository` |
| Интерфейсы → ядро | 4 компонента | 1 фасад |
| Executor → Notification | прямой вызов | через EventBus |
| Store → SQLite | жёстко | через абстракцию `Database` |
| Risk | монолит | композиция правил |
| Strategy | считает и исполняет | чистая функция |

#### 5.7.12. Цена

- **Больше компонентов.** Каждый проще, но карту держать в голове сложнее — 5.1 вырастет на семь строк: четыре репозитория, `Database`, `EventBus`, `InvestApp`.
- **Больше кода.** Абстракции стоят строк, и на одном человеке это заметнее, чем в команде.
- **Косвенность.** События и фасад скрывают поток управления: чтобы найти, кто обработал `OrderPlaced`, нужен поиск по подписке, а не по имени метода.

Для solo-проекта из PROJECT_PLAN это размен не в пользу абстракций «на всякий случай» — отсюда порядок ниже.

#### 5.7.13. Порядок внедрения

Не переписывать всё сразу. Шаг 1 даёт максимум эффекта при минимуме затрат, шаги 4–6 оправданы только при росте.

| Шаг | Что | Когда оправдан |
|---|---|---|
| 1 | Фасад `InvestApp` | сразу: 80% эффекта за 20% усилий |
| 2 | Доменные структуры вместо `dict` | сразу: стабилизирует контракты 5.3 |
| 3 | Разделение Store на репозитории | когда доменных компонентов станет много |
| 4 | EventBus для Notification и Journal | когда появится автоматическая торговля (этап 2.3) |
| 5 | Абстракция `Database` | когда захочется заменить SQLite |
| 6 | Risk как композиция правил | когда правил станет больше трёх |

Шаги 1 и 2 не конфликтуют с порядком сборки MVP из 5.5: `InvestApp` — это то, что в 5.5 стоит на месте Presenter и CLI, а доменные структуры — то, чем они обмениваются.
